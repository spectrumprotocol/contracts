...
pub fn query_all_balances(deps: Deps<TerraQuery>, account_addr: Addr) -> StdResult<Vec<Coin>> {
...
pub fn query_balance(deps: Deps<TerraQuery>, account_addr: Addr, denom: String) -> StdResult<Uint256> {
    // load price form the oracle
...
pub fn query_supply(deps: Deps<TerraQuery>, contract_addr: Addr) -> StdResult<Uint256> {
    // load price form the oracle
...
pub fn query_tax_rate_and_cap(deps: Deps<TerraQuery>, denom: String) -> StdResult<(Decimal256, Uint256)> {
...
pub fn query_tax_rate(deps: Deps<TerraQuery>) -> StdResult<Decimal256> {
    let terra_querier = TerraQuerier::new(&deps.querier);
    Ok(terra_querier.query_tax_rate()?.rate.into())
}

pub fn compute_tax(deps: Deps<TerraQuery>, coin: &Coin) -> StdResult<Uint256> {
    let terra_querier = TerraQuerier::new(&deps.querier);
    ...
    Ok(std::cmp::min(
        amount * Decimal256::one() - amount * (Decimal256::one() + tax_rate).inv().ok_or_else(|| StdError::generic_err("Invert 0"))?,
        tax_cap,
    ))
}

pub fn deduct_tax(deps: Deps<TerraQuery>, coin: Coin) -> StdResult<Coin> {
