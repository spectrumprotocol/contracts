...
const NEXUS_TOKEN: &str = "psi_token";
const NASSET_STAKING: &str = "nasset_staking";
const NASSET_LP: &str = "nasset_lp";
const NASSET_TOKEN: &str = "nasset_token";
const NASSET_POOL: &str = "nasset_pool";
    ...
    deps.querier.with_terraswap_pairs(&[
        (
            ...
            &PairInfo {
                ...
                liquidity_token: PSI_LP.to_string(),
                asset_decimals: [6, 6],
            },
        ),
        (
        ...
        (
            ...
            &PairInfo {
                ...
                contract_addr: NASSET_POOL.to_string(),
                liquidity_token: NASSET_LP.to_string(),
                asset_decimals: [6, 6],
            },
        ),
    ]);
...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        pair_contract: NASSET_POOL.to_string(),
        ust_pair_contract: PSI_POOL.to_string(),
    };
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    ...
    let msg = ExecuteMsg::register_asset {
        ...
        staking_token: NASSET_LP.to_string(),
        weight: 1u32,
    };
    let res = execute(deps.as_mut(), env.clone(), info, msg.clone());
    assert!(res.is_err());
...
// fn test_compound_psi_from_allowance(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
//     let env = mock_env();
//     let info = mock_info(TEST_CONTROLLER, &[]);
//
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    ...
    // bond success user1 1000 nAsset-LP
    let info = mock_info(NASSET_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    // bond user2 5000 nAsset-LP auto-stake
    let info = mock_info(NASSET_LP, &[]);
    let msg = ExecuteMsg::receive(Cw20ReceiveMsg {
...
fn test_compound_psi(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
...
fn test_compound_psi_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    // update fees
    let env = mock_env();
    let info = mock_info(SPEC_GOV, &[]);
    let msg = ExecuteMsg::update_config {
