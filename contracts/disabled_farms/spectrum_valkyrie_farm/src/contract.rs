...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            valkyrie_token: deps.api.addr_canonicalize(&msg.valkyrie_token)?,
            valkyrie_staking: deps.api.addr_canonicalize(&msg.valkyrie_staking)?,
            valkyrie_gov: deps.api.addr_canonicalize(&msg.valkyrie_gov)?,
...
fn query_config(deps: Deps) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        valkyrie_staking: deps.api.addr_humanize(&config.valkyrie_staking)?.to_string(),
        spectrum_gov: deps.api.addr_humanize(&config.spectrum_gov)?.to_string(),
        valkyrie_gov: deps.api.addr_humanize(&config.valkyrie_gov)?.to_string(),
