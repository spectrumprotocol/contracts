...
use spectrum_protocol::spec_astroport_farm::{
...
pub fn instantiate(
    ...
    config_store(deps.storage).save(&Config {
        ...
        generator_proxy: if let Some(generator_proxy) = &msg.generator_proxy {
            Some(deps.api.addr_canonicalize(&generator_proxy)?)
        } else {
            None
        },
    })?;
...
fn update_config(
    ...
    if let Some(generator_proxy) = generator_proxy {
        config.generator_proxy = Some(deps.api.addr_canonicalize(&generator_proxy)?);
    }
...
fn query_config(deps: Deps<TerraQuery>) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        generator_proxy: if let Some(generator_proxy) = &config.generator_proxy {
            Some(deps.api.addr_humanize(&generator_proxy)?.to_string())
        } else {
            None
        },
    };
