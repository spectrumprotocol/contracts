...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            terrafloki_factory: deps.api.addr_canonicalize(&msg.terrafloki_factory)?,
            spectrum_token: deps.api.addr_canonicalize(&msg.spectrum_token)?,
            spectrum_gov: deps.api.addr_canonicalize(&msg.spectrum_gov)?,
            terrafloki_token: deps.api.addr_canonicalize(&msg.terrafloki_token)?,
            terrafloki_ticket: deps.api.addr_canonicalize(&msg.terrafloki_ticket)?,
            terrafloki_staking: deps.api.addr_canonicalize(&msg.terrafloki_staking)?,
            terrafloki_ticket_farm: deps.api.addr_canonicalize(&msg.terrafloki_ticket_farm)?,
...
fn query_config(deps: Deps) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        terrafloki_factory: deps
            .api
            .addr_humanize(&config.terrafloki_factory)?
            .to_string(),
        spectrum_token: deps.api.addr_humanize(&config.spectrum_token)?.to_string(),
        terrafloki_token: deps.api.addr_humanize(&config.terrafloki_token)?.to_string(),
        terrafloki_ticket: deps.api.addr_humanize(&config.terrafloki_ticket)?.to_string(),
        terrafloki_staking: deps.api.addr_humanize(&config.terrafloki_staking)?.to_string(),
        spectrum_gov: deps.api.addr_humanize(&config.spectrum_gov)?.to_string(),
        terrafloki_ticket_farm: deps.api.addr_humanize(&config.terrafloki_ticket_farm)?.to_string(),
