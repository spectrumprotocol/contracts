...
use terrafloki::ticket_farm::ExecuteMsg as TerraflokiTicketFarmExecuteMsg;
use terrafloki::staking::ExecuteMsg as TerraflokiStakingExecuteMsg;
...
use spectrum_protocol::terrafloki_farm::{
...
use std::fmt::Debug;
use terrafloki::asset::{Asset, AssetInfo, PairInfo};
use terrafloki::pair::{Cw20HookMsg as TerraswapCw20HookMsg, ExecuteMsg as TerraswapExecuteMsg};
...
const TFLOKI_FARM: &str = "tfloki_farm";
const TFLOKI_TOKEN: &str = "tfloki_token";
const TFLOKI_TICKET: &str = "tfloki_token";
const TFLOKI_STAKING: &str = "tfloki_staking";
const TFLOKI_LP: &str = "tfloki_lp";
const TFLOKI_POOL: &str = "tfloki_pool";
const TFLOKI_FACTORY: &str = "tfloki_factory";
...
fn test() {
    ...
    deps.querier.with_terraswap_pairs(&[
        (
            &"uusdtfloki_token".to_string(),
            &PairInfo {
                asset_infos: [
                    AssetInfo::Token {
                        contract_addr: TFLOKI_TOKEN.to_string(),
                    },
                    AssetInfo::NativeToken {
                        denom: "uusd".to_string(),
                    },
                ],
                contract_addr: TFLOKI_POOL.to_string(),
                liquidity_token: TFLOKI_LP.to_string(),
            },
        ),
        (
    ...
    test_compound_tfloki_from_allowance(&mut deps);
    test_bond(&mut deps);
    test_compound_tfloki(&mut deps);
    test_compound_tfloki_with_fees(&mut deps);
}
...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        terrafloki_ticket_farm: TFLOKI_FARM.to_string(),
        terrafloki_token: TFLOKI_TOKEN.to_string(),
        terrafloki_ticket: TFLOKI_TICKET.to_string(),
        terrafloki_staking: TFLOKI_STAKING.to_string(),
        terrafloki_factory: TFLOKI_FACTORY.to_string(),
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    let msg = ExecuteMsg::register_asset {
        asset_token: TFLOKI_TOKEN.to_string(),
        staking_token: TFLOKI_LP.to_string(),
        weight: 1u32,
    };
    let res = execute(deps.as_mut(), env.clone(), info, msg.clone());
    assert!(res.is_err());
    ...
    assert_eq!(
        ...
        vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: TFLOKI_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&TerraflokiStakingExecuteMsg::Withdraw {}).unwrap(),
            }),
        ]
    );
}

fn test_compound_tfloki_from_allowance(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);

    let asset_token_raw = deps.api.addr_canonicalize(&TFLOKI_TOKEN.to_string()).unwrap();
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                msg: to_binary(&Cw20ExecuteMsg::Send {
                    contract: TFLOKI_POOL.to_string(),
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    // bond success user1 1000 TFLOKI-LP
    let info = mock_info(TFLOKI_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    let mut pool_info = pool_info_read(deps_ref.storage)
        .load(config.terrafloki_token.as_slice())
        .unwrap();
    deposit_farm_share(
        deps_ref,
        &mut state,
    ...
    pool_info_store(deps.as_mut().storage)
        .save(config.terrafloki_token.as_slice(), &pool_info)
        .unwrap();
    deps.querier.with_token_balances(&[
        (
            &TFLOKI_STAKING.to_string(),
            &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(10000u128))],
        ),
        (
            &TFLOKI_FARM.to_string(),
    ...
    assert_eq!(
        ...
        [
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: TFLOKI_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&TerraflokiStakingExecuteMsg::Unbond {
                    amount: Uint128::from(3000u128),
                })
                .unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: TFLOKI_LP.to_string(),
                funds: vec![],
    ...
    assert_eq!(
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: TFLOKI_FARM.to_string(),
                funds: vec![],
                msg: to_binary(&TerraflokiTicketFarmExecuteMsg::Unbond {
    ...
    // bond user2 5000 TFLOKI-LP auto-stake
    let info = mock_info(TFLOKI_LP, &[]);
    let msg = ExecuteMsg::receive(Cw20ReceiveMsg {
...
fn test_compound_tfloki(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    /*
    pending rewards 12000 TFLOKI
    USER1 7000 (auto 4200, stake 2800)
    USER2 5000 (auto 0, stake 5000)
    total 12000
    auto 4200 / 12000 * 12000 = 4200
    stake 7800 / 12000 * 12000 = 7800
    swap amount 2100 TFLOKI -> 2073 UST
    provide UST = 2052
    provide TFLOKI = 2052
    remaining = 48
    */
    ...
    assert_eq!(
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                funds: vec![],
                msg: to_binary(&Cw20ExecuteMsg::Send {
                    contract: TFLOKI_FARM.to_string(),
                    amount: Uint128::from(7800u128),
                    msg: to_binary(&TerraflokiTicketFarmCw20HookMsg::Bond {}).unwrap(),
                })
                .unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
...
fn test_compound_tfloki_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    // update fees
    let env = mock_env();
    let info = mock_info(SPEC_GOV, &[]);
    let msg = ExecuteMsg::update_config {
    ...
    swap amount 2042 TFLOKI -> 2016 UST
    provide UST = 1996
    provide TFLOKI = 1996
    remaining = 46
    fee swap amount 605 TFLOKI -> 591 UST -> 590 SPEC
    community fee = 363 / 605 * 590 = 354
    platform fee = 121 / 605 * 590 = 118
    controller fee = 121 / 605 * 590 = 118
    total swap amount 2647 TFLOKI
    */
    let msg = ExecuteMsg::compound {};
    let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
