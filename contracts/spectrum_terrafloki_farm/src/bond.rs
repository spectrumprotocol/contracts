...
use crate::querier::query_terrafloki_pool_balance;
use terrafloki::ticket_farm::{
    ExecuteMsg as TerraflokiTicketFarmExecuteMsg, QueryMsg as TerraflokiTicketFarmQueryMsg,
    StakerInfoResponse as TerraflokiTicketFarmStakerResponse,
};
use terrafloki::staking::{
    Cw20HookMsg as TerraflokiCw20HookMsg, ExecuteMsg as TerraflokiStakingExecuteMsg,
};
use spectrum_protocol::terrafloki_farm::{RewardInfoResponse, RewardInfoResponseItem};
...
pub fn bond(
    ...
    let lp_balance = query_terrafloki_pool_balance(
        deps.as_ref(),
...
pub fn deposit_farm_share(
    ...
    let staked: TerraflokiTicketFarmStakerResponse =
        deps.querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
            contract_addr: deps.api.addr_humanize(&config.terrafloki_ticket_farm)?.to_string(),
            msg: to_binary(&TerraflokiTicketFarmQueryMsg::StakerInfo {
...
// stake LP token to Terrafloki Staking
fn stake_token(
    api: &dyn Api,
    ...
    let terrafloki_staking = api.addr_humanize(&terrafloki_staking)?;
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::Send {
                ...
                amount,
                msg: to_binary(&TerraflokiCw20HookMsg::Bond {})?,
            })?,
        })])
        .add_attributes(vec![
            attr("action", "bond"),
    ...
    Ok(Response::new()
        .add_messages(vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps.api.addr_humanize(&config.terrafloki_staking)?.to_string(),
                funds: vec![],
                msg: to_binary(&TerraflokiStakingExecuteMsg::Unbond { amount })?,
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps
                    .api
...
pub fn withdraw(
    ...
    if !farm_amount.is_zero() {
        ...
        messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps.api.addr_humanize(&config.terrafloki_token)?.to_string(),
            msg: to_binary(&Cw20ExecuteMsg::Transfer {
                recipient: info.sender.to_string(),
                amount: farm_amount,
            })?,
...
fn withdraw_reward(
    ...
    let farm_staked: TerraflokiTicketFarmStakerResponse =
        deps.querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
            contract_addr: deps.api.addr_humanize(&config.terrafloki_ticket_farm)?.to_string(),
            msg: to_binary(&TerraflokiTicketFarmQueryMsg::StakerInfo {
                staker: deps.api.addr_humanize(&state.contract_addr)?.to_string(),
...
fn read_reward_infos(
    ...
    let lp_balance =
        query_terrafloki_pool_balance(deps, &config.terrafloki_staking, &state.contract_addr)?;
