...
use glow::gov::StakerResponse as GlowStakerResponse;
use glow::staking::StakerInfoResponse as GlowStakerInfoResponse;
            ...
            QueryRequest::Wasm(WasmQuery::Smart { contract_addr, msg }) => {
                match from_binary(msg).unwrap() {
                    ...
                    MockQueryMsg::Staker { address } => {
                        let balance = self.read_token_balance(contract_addr, address);
                        SystemResult::Ok(ContractResult::from(to_binary(&GlowStakerResponse {
                            balance,
                            share: balance
                                .multiply_ratio(100u128, self.token_querier.balance_percent)
                        })))
