...
use crate::querier::query_glow_reward_info;
...
pub fn compound(deps: DepsMut, env: Env, info: MessageInfo) -> StdResult<Response> {
    ...
    let glow_staking = deps.api.addr_humanize(&config.glow_staking)?;
    let glow_token = deps.api.addr_humanize(&config.glow_token)?;
    let glow_gov = deps.api.addr_humanize(&config.glow_gov)?;

    let glow_reward_info = query_glow_reward_info(
        deps.as_ref(),
        &config.glow_staking,
    ...
    let mut total_glow_swap_amount = Uint128::zero();
    let mut total_glow_stake_amount = Uint128::zero();
    let mut total_glow_commission = Uint128::zero();
    ...
    // calculate auto-compound, auto-Stake, and commission in GLOW
    let mut pool_info = pool_info_read(deps.storage).load(config.glow_token.as_slice())?;
    let reward = glow_reward_info.pending_reward;
    if !reward.is_zero() && !glow_reward_info.bond_amount.is_zero() {
        let commission = reward * total_fee;
        let glow_amount = reward.checked_sub(commission)?;
        // add commission to total swap amount
        total_glow_commission += commission;
        total_glow_swap_amount += commission;

        let auto_bond_amount = glow_reward_info
            .bond_amount
            .checked_sub(pool_info.total_stake_bond_amount)?;
        compound_amount =
            glow_amount.multiply_ratio(auto_bond_amount, glow_reward_info.bond_amount);
        let stake_amount = glow_amount.checked_sub(compound_amount)?;
        ...
        total_glow_stake_amount += stake_amount;
    }
    ...
    deposit_farm_share(
        ...
        &mut pool_info,
        &config,
        total_glow_stake_amount,
    )?;
    state_store(deps.storage).save(&state)?;
    pool_info_store(deps.storage).save(config.glow_token.as_slice(), &pool_info)?;

    // get reinvest amount
    let reinvest_allowance = query_token_balance(&deps.querier, glow_token.clone(), env.contract.address.clone())?;
    ...
    // add commission to reinvest GLOW to total swap amount
    total_glow_swap_amount += swap_amount;
    ...
    let glow = Asset {
        ...
        amount: total_glow_swap_amount,
    };
    let glow_swap_rate = simulate(
        &deps.querier,
        pair_contract.clone(),
        &glow,
    )?;
    let total_ust_return_amount = deduct_tax(&deps.querier, glow_swap_rate.return_amount, config.base_denom.clone())?;
    attributes.push(attr("total_ust_return_amount", total_ust_return_amount));

    let total_ust_commission_amount = if total_glow_swap_amount != Uint128::zero() {
        total_ust_return_amount.multiply_ratio(total_glow_commission, total_glow_swap_amount)
    } else {
        Uint128::zero()
    };
    ...
    let provide_glow = compute_provide_after_swap(
        &pool,
        &glow,
        glow_swap_rate.return_amount,
        net_reinvest_ust
    )?;

    let mut messages: Vec<CosmosMsg> = vec![];
    let withdraw_all_glow: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: glow_staking.to_string(),
        funds: vec![],
        msg: to_binary(&GlowStakingExecuteMsg::Withdraw {})?,
    });
    messages.push(withdraw_all_glow);

    if !total_glow_swap_amount.is_zero() {
        let swap_glow: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: glow_token.to_string(),
    ...
    if !total_glow_stake_amount.is_zero() {
        let stake_glow = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: glow_token.to_string(),
            funds: vec![],
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: glow_gov.to_string(),
                amount: total_glow_stake_amount,
                msg: to_binary(&GlowGovCw20HookMsg::StakeVotingTokens {})?,
            })?,
        });
        messages.push(stake_glow);
    }

    if !provide_glow.is_zero() {
    ...
    attributes.push(attr("asset_token", glow_token));
    attributes.push(attr("reinvest_amount", reinvest_amount));
    attributes.push(attr("provide_token_amount", provide_glow));
...
pub fn stake(
    ...
    // only glow farm contract can execute this message
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            funds: vec![],
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: glow_staking.to_string(),
                amount,
                msg: to_binary(&GlowStakingCw20HookMsg::Bond {})?,
            })?,
        })])
        .add_attributes(vec![
            attr("action", "stake"),
