...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            glow_staking: deps.api.addr_canonicalize(&msg.glow_staking)?,
            glow_gov: deps.api.addr_canonicalize(&msg.glow_gov)?,
...
fn query_config(deps: Deps) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        glow_staking: deps.api.addr_humanize(&config.glow_staking)?.to_string(),
        spectrum_gov: deps.api.addr_humanize(&config.spectrum_gov)?.to_string(),
        glow_gov: deps.api.addr_humanize(&config.glow_gov)?.to_string(),
