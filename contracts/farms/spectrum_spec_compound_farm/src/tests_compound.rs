...
use spectrum_protocol::spec_farm::{ExecuteMsg as SpecStakingExecuteMsg};
use schemars::JsonSchema;
use serde::{Deserialize, Serialize};
use spectrum_protocol::gov::{ExecuteMsg as GovExecuteMsg};
use spectrum_protocol::spec_compound_farm::{ConfigInfo, ExecuteMsg, PoolItem, PoolsResponse, QueryMsg, StateInfo, Cw20HookMsg};
...
fn test() {
    ...
    test_compound_spec_from_allowance(&mut deps);
    test_bond(&mut deps);
    test_compound_spec(&mut deps);
    test_compound_spec_with_fees(&mut deps);
}
...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        spectrum_staking: SPEC_STAKING.to_string(),
        ...
        pair_contract: SPEC_POOL.to_string(),
    };
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    let msg = ExecuteMsg::register_asset {
        asset_token: SPEC_TOKEN.to_string(),
        staking_token: SPEC_LP.to_string(),
        weight: 1u32,
    };
    let res = execute(deps.as_mut(), env.clone(), info, msg.clone());
    assert!(res.is_err());
    ...
    assert_eq!(
        ...
        vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: SPEC_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&SpecStakingExecuteMsg::withdraw { asset_token: Some(SPEC_TOKEN.to_string()), spec_amount: None }).unwrap(),
            }),
        ]
    );
}

fn test_compound_spec_from_allowance(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);

    deps.querier.with_token_balances(&[
        (&SPEC_TOKEN.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(100_000_000u128))])
    ]);
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    // bond success user1 1000 SPEC-LP
    let info = mock_info(SPEC_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    let mut pool_info = pool_info_read(deps_ref.storage)
        .load(config.spectrum_token.as_slice())
        .unwrap();
        deposit_farm_share(
            deps_ref,
            &env,
    ...
    pool_info_store(deps.as_mut().storage)
        .save(config.spectrum_token.as_slice(), &pool_info)
        .unwrap();
    deps.querier.with_token_balances(&[
        (
            &SPEC_STAKING.to_string(),
    ...
    assert_eq!(
        ...
        [
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: SPEC_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&SpecStakingExecuteMsg::unbond {
                    asset_token: SPEC_TOKEN.to_string(),
                    amount: Uint128::from(3000u128),
                }).unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: SPEC_LP.to_string(),
                funds: vec![],
    ...
    // bond user2 5000 SPEC-LP auto-compound
    let info = mock_info(SPEC_LP, &[]);
    let msg = ExecuteMsg::receive(Cw20ReceiveMsg {
...
fn test_compound_spec(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    /*
    pending rewards 12000 SPEC
    USER1 7000 (auto 7000, stake 0)
    USER2 5000 (auto 5000, stake 0)
    total 12000
    auto 12000 / 12000 * 12000 = 12000
    stake 0 / 12000 * 12000 = 0
    swap amount 6000 SPEC -> 5982 UST
    provide UST = 5863
    provide SPEC = 5863
    remaining = 137
    */
...
fn test_compound_spec_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    // update fees
    let env = mock_env();
    let info = mock_info(SPEC_GOV, &[]);
    let msg = ExecuteMsg::update_config {
    ...
    swap amount 6532 SPEC (12100 / 2 + 605) -> 6512 UST
    provide UST = 5616
    provide SPEC = 5616
    remaining = 311
    fee swap amount 605 SPEC -> 591 UST -> 590 SPEC
    community fee = 363 / 605 * 590 = 354
    platform fee = 121 / 605 * 590 = 118
    controller fee = 121 / 605 * 590 = 118
    total swap amount 6532 SPEC
    */
    let msg = ExecuteMsg::compound {};
    let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
