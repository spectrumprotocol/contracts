...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            terraworld_token: deps.api.addr_canonicalize(&msg.terraworld_token)?,
            terraworld_staking: deps.api.addr_canonicalize(&msg.terraworld_staking)?,
            terraworld_gov: deps.api.addr_canonicalize(&msg.terraworld_gov)?,
...
fn query_config(deps: Deps) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        terraworld_token: deps.api.addr_humanize(&config.terraworld_token)?.to_string(),
        terraworld_staking: deps.api.addr_humanize(&config.terraworld_staking)?.to_string(),
        spectrum_gov: deps.api.addr_humanize(&config.spectrum_gov)?.to_string(),
        terraworld_gov: deps.api.addr_humanize(&config.terraworld_gov)?.to_string(),
