...
fn test() {
    ...
    deps.querier.with_terraswap_pairs(&[
        (
            ...
            &PairInfo {
                ...
                liquidity_token: TWD_LP.to_string(),
                asset_decimals: [6, 6],
            },
        ),
        (
    ...
    test_compound_twd_from_allowance(&mut deps);
    test_bond(&mut deps);
...
fn test_compound_twd_from_allowance(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);

    deps.querier.with_token_balances(&[
        (&TWD_TOKEN.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(100_000_000u128))])
    ]);
...
fn test_compound_twd(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    /*
    pending rewards 12000 TWD
    USER1 7000 (auto 4200, stake 2800)
    USER2 5000 (auto 0, stake 5000)
    total 12000
    auto 4200 / 12000 * 12000 = 4200
    stake 7800 / 12000 * 12000 = 7800
    swap amount 2100 TWD -> 2073 UST
    provide UST = 2052
    provide TWD = 2052
    remaining = 48
    */
...
fn test_compound_twd_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    // update fees
    let env = mock_env();
    let info = mock_info(SPEC_GOV, &[]);
    let msg = ExecuteMsg::update_config {
...
fn test_compound_twd_with_fees_but_not_compound_gov(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>){
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    swap amount 2042 TWD -> 2016 UST
    provide UST = 1996
    provide TWD = 1996
    remaining = 46
    fee swap amount 605 TWD -> 591 UST -> 590 SPEC
