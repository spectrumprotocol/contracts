...
use terra_name_service::gov::StakerResponse as TerraNameServiceStakerResponse;
use terra_name_service::staking::StakerInfoResponse as TerraNameServiceStakerInfoResponse;
            ...
            QueryRequest::Wasm(WasmQuery::Smart { contract_addr, msg }) => {
                match from_binary(msg).unwrap() {
                    ...
                    MockQueryMsg::Staker { address } => {
                        let balance = self.read_token_balance(contract_addr, address);
                        SystemResult::Ok(ContractResult::from(to_binary(&TerraNameServiceStakerResponse {
                            balance,
                            share: balance
                                .multiply_ratio(100u128, self.token_querier.balance_percent),
                            locked_balance: vec![],
                    ...
                    MockQueryMsg::StakerInfo {
                        ...
                        let contract_addr = &TNS_STAKING.to_string();
                        let balance = self.read_token_balance(contract_addr, staker.clone());
                        SystemResult::Ok(ContractResult::from(to_binary(
                            &TerraNameServiceStakerInfoResponse {
                                staker,
