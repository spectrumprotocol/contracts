...
use terra_name_service::gov::ExecuteMsg as TerraNameServiceGovExecuteMsg;
use terra_name_service::staking::ExecuteMsg as TerraNameServiceStakingExecuteMsg;
...
const TNS_GOV: &str = "tns_gov";
const TNS_TOKEN: &str = "tns_token";
const TNS_STAKING: &str = "tns_staking";
const TNS_LP: &str = "tns_lp";
const TNS_POOL: &str = "tns_pool";
...
const ANC_MARKET: &str = "tns_market";
...
fn test() {
    ...
    deps.querier.with_terraswap_pairs(&[
        (
            &"uusdtns_token".to_string(),
            &PairInfo {
                asset_infos: [
                    AssetInfo::Token {
                        contract_addr: TNS_TOKEN.to_string(),
                    },
                    AssetInfo::NativeToken {
                        denom: "uusd".to_string(),
                    },
                ],
                contract_addr: TNS_POOL.to_string(),
                liquidity_token: TNS_LP.to_string(),
                asset_decimals: [6, 6],
            },
        ),
        (
    ...
    test_compound_tns_from_allowance(&mut deps);
    test_bond(&mut deps);
    test_compound_tns(&mut deps);
    test_compound_tns_with_fees(&mut deps);
}
...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        terra_name_service_gov: TNS_GOV.to_string(),
        terra_name_service_token: TNS_TOKEN.to_string(),
        terra_name_service_staking: TNS_STAKING.to_string(),
        ...
        pair_contract: TNS_POOL.to_string(),
    };
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    ...
    let msg = ExecuteMsg::register_asset {
        asset_token: TNS_TOKEN.to_string(),
        staking_token: TNS_LP.to_string(),
        weight: 1u32,
    };
    let res = execute(deps.as_mut(), env.clone(), info, msg.clone());
    assert!(res.is_err());
    ...
    assert_eq!(
        ...
        vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: TNS_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&TerraNameServiceStakingExecuteMsg::Withdraw {}).unwrap(),
            }),
        ]
    );
}

fn test_compound_tns_from_allowance(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);

    deps.querier.with_token_balances(&[
        (&TNS_TOKEN.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(100_000_000u128))])
    ]);
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    ...
    // bond success user1 1000 TNS-LP
    let info = mock_info(TNS_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    let mut pool_info = pool_info_read(deps_ref.storage)
        .load(config.terra_name_service_token.as_slice())
        .unwrap();
    deposit_farm_share(
        deps_ref,
        &env,
    ...
    pool_info_store(deps.as_mut().storage)
        .save(config.terra_name_service_token.as_slice(), &pool_info)
        .unwrap();
    deps.querier.with_token_balances(&[
        (
            &TNS_STAKING.to_string(),
    ...
    assert_eq!(
        ...
        [
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: TNS_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&TerraNameServiceStakingExecuteMsg::Unbond {
                    amount: Uint128::from(3000u128),
                })
                .unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: TNS_LP.to_string(),
                funds: vec![],
    ...
    assert_eq!(
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: TNS_GOV.to_string(),
                funds: vec![],
                msg: to_binary(&TerraNameServiceGovExecuteMsg::WithdrawVotingTokens {
    ...
    // bond user2 5000 TNS-LP auto-stake
    let info = mock_info(TNS_LP, &[]);
    let msg = ExecuteMsg::receive(Cw20ReceiveMsg {
...
fn test_compound_tns(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    /*
    pending rewards 12000 TNS
    USER1 7000 (auto 4200, stake 2800)
    USER2 5000 (auto 0, stake 5000)
    total 12000
    auto 4200 / 12000 * 12000 = 4200
    stake 7800 / 12000 * 12000 = 7800
    swap amount 2100 TNS -> 2073 UST
    provide UST = 2052
    provide TNS = 2052
    remaining = 48
    */
    ...
    assert_eq!(
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                funds: vec![],
                msg: to_binary(&Cw20ExecuteMsg::Send {
                    contract: TNS_GOV.to_string(),
                    amount: Uint128::from(7800u128),
                    msg: to_binary(&TerraNameServiceGovCw20HookMsg::StakeVotingTokens {}).unwrap(),
                })
                .unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
...
fn test_compound_tns_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    // update fees
    let env = mock_env();
    let info = mock_info(SPEC_GOV, &[]);
    let msg = ExecuteMsg::update_config {
    ...
    swap amount 2042 TNS -> 2016 UST
    provide UST = 1996
    provide TNS = 1996
    remaining = 46
    fee swap amount 605 TNS -> 591 UST -> 590 SPEC
    community fee = 363 / 605 * 590 = 354
    platform fee = 121 / 605 * 590 = 118
    controller fee = 121 / 605 * 590 = 118
    total swap amount 2647 TNS
    */
    let msg = ExecuteMsg::compound {};
    let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
