...
use valkyrie::governance::execute_msgs::ExecuteMsg as ValkyrieGovExecuteMsg;
...
fn test() {
    ...
    test_compound_vkr_from_allowance(&mut deps);
    test_bond(&mut deps);
    test_compound_vkr(&mut deps);
...
fn test_compound_vkr_from_allowance(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);

    deps.querier.with_token_balances(&[
        (&VKR_TOKEN.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(100_000_000u128))])
    ]);
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    // bond success user1 1000 VKR-LP
    let info = mock_info(VKR_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    let mut pool_info = pool_info_read(deps_ref.storage)
        .load(config.valkyrie_token.as_slice())
        .unwrap();
    deposit_farm_share(
        deps_ref,
        &env,
    ...
    pool_info_store(deps.as_mut().storage)
        .save(config.valkyrie_token.as_slice(), &pool_info)
        .unwrap();
    deps.querier.with_token_balances(&[
        (
    ...
    assert_eq!(
        ...
        [
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: VKR_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&ValkyrieStakingExecuteMsg::Unbond {
                    amount: Uint128::from(3000u128),
                })
                .unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: VKR_LP.to_string(),
                funds: vec![],
    ...
    assert_eq!(
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: VKR_GOV.to_string(),
                funds: vec![],
                msg: to_binary(&ValkyrieGovExecuteMsg::UnstakeGovernanceToken {
    ...
    // bond user2 5000 VKR-LP auto-stake
    let info = mock_info(VKR_LP, &[]);
    let msg = ExecuteMsg::receive(Cw20ReceiveMsg {
...
fn test_compound_vkr(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    /*
    pending rewards 12000 VKR
    USER1 7000 (auto 4200, stake 2800)
    USER2 5000 (auto 0, stake 5000)
    total 12000
    auto 4200 / 12000 * 12000 = 4200
    stake 7800 / 12000 * 12000 = 7800
    swap amount 2100 VKR -> 2073 UST
    provide UST = 2052
    provide VKR = 2052
    remaining = 48
    */
    ...
    assert_eq!(
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                funds: vec![],
                msg: to_binary(&Cw20ExecuteMsg::Send {
                    contract: VKR_GOV.to_string(),
                    amount: Uint128::from(7800u128),
                    msg: to_binary(&ValkyrieGovCw20HookMsg::StakeGovernanceToken {}).unwrap(),
                })
                .unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
...
fn test_compound_vkr_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    swap amount 2042 VKR -> 2016 UST
    provide UST = 1996
    provide VKR = 1996
    remaining = 46
    fee swap amount 605 VKR -> 591 UST -> 590 SPEC
    community fee = 363 / 605 * 590 = 354
    platform fee = 121 / 605 * 590 = 118
    controller fee = 121 / 605 * 590 = 118
    total swap amount 2647 VKR
    */
    let msg = ExecuteMsg::compound {};
    let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
