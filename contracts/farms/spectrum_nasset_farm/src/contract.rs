...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            nasset_rewards: deps.api.addr_canonicalize(&msg.nasset_rewards)?,
            nasset_vault: deps.api.addr_canonicalize(&msg.nasset_vault)?,
...
fn query_config(deps: Deps<TerraQuery>) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        nasset_rewards: deps.api.addr_humanize(&config.nasset_rewards)?.to_string(),
        nasset_vault: deps.api.addr_humanize(&config.nasset_vault)?.to_string(),
