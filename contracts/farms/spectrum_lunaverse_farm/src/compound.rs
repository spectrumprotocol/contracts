...
use crate::querier::{query_lunaverse_reward_info};

use cw20::Cw20ExecuteMsg;

use crate::state::{pool_info_read, pool_info_store, read_state, Config, PoolInfo};
use crate::lunaverse_staking::{
    Cw20HookMsg as LunaverseStakingCw20HookMsg, ExecuteMsg as LunaverseStakingExecuteMsg,
};
...
pub fn compound(deps: DepsMut, env: Env, info: MessageInfo) -> StdResult<Response> {
    ...
    let lunaverse_staking = deps.api.addr_humanize(&config.lunaverse_staking)?;
    let lunaverse_token = deps.api.addr_humanize(&config.lunaverse_token)?;
    let lunaverse_gov = deps.api.addr_humanize(&config.lunaverse_gov)?;

    let lunaverse_reward_info = query_lunaverse_reward_info(
        deps.as_ref(),
        &config.lunaverse_staking,
        &env.contract.address
    )?;

    let mut total_luv_swap_amount = Uint128::zero();
    let mut total_luv_stake_amount = Uint128::zero();
    let mut total_luv_commission = Uint128::zero();
    ...
    // calculate auto-compound, auto-Stake, and commission in LUV
    let mut pool_info = pool_info_read(deps.storage).load(config.lunaverse_token.as_slice())?;
    let reward = lunaverse_reward_info.pending_reward;
    if !reward.is_zero() && !lunaverse_reward_info.bond_amount.is_zero() {
        let commission = reward * total_fee;
        let luv_amount = reward.checked_sub(commission)?;
        // add commission to total swap amount
        total_luv_commission += commission;
        total_luv_swap_amount += commission;

        let auto_bond_amount = lunaverse_reward_info
            .bond_amount
            .checked_sub(pool_info.total_stake_bond_amount)?;
        compound_amount =
            luv_amount.multiply_ratio(auto_bond_amount, lunaverse_reward_info.bond_amount);
        let stake_amount = luv_amount.checked_sub(compound_amount)?;
        ...
        total_luv_stake_amount += stake_amount;
    }
    ...
    deposit_farm_share(
        ...
        &mut pool_info,
        &config,
        total_luv_stake_amount,
        Some(env.block.time.seconds())
    )?;
    state_store(deps.storage).save(&state)?;
    pool_info_store(deps.storage).save(config.lunaverse_token.as_slice(), &pool_info)?;

    // get reinvest amount
    let reinvest_allowance = query_token_balance(&deps.querier, lunaverse_token.clone(), env.contract.address.clone())?;
    ...
    // add commission to reinvest LUV to total swap amount
    total_luv_swap_amount += swap_amount;

    // find LUV swap rate
    let luv = Asset {
        info: AssetInfo::Token {
            contract_addr: lunaverse_token.to_string(),
        },
        amount: total_luv_swap_amount,
    };
    let luv_swap_rate = simulate(
        &deps.querier,
        pair_contract.clone(),
        &luv,
    )?;

    let total_ust_return_amount = deduct_tax(&deps.querier, luv_swap_rate.return_amount, config.base_denom.clone())?;
    attributes.push(attr("total_ust_return_amount", total_ust_return_amount));

    let total_ust_commission_amount = if total_luv_swap_amount != Uint128::zero() {
        total_ust_return_amount.multiply_ratio(total_luv_commission, total_luv_swap_amount)
    } else {
        Uint128::zero()
    };
    ...
    let provide_luv = compute_provide_after_swap(
        &pool,
        &luv,
        luv_swap_rate.return_amount,
        net_reinvest_ust
    )?;

    let mut messages: Vec<CosmosMsg> = vec![];
    let withdraw_all_luv: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: lunaverse_staking.to_string(),
        funds: vec![],
        msg: to_binary(&LunaverseStakingExecuteMsg::Withdraw {})?,
    });
    messages.push(withdraw_all_luv);

    if !total_luv_swap_amount.is_zero() {
        let swap_luv: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: lunaverse_token.to_string(),
    ...
    if !total_luv_stake_amount.is_zero() {
        let stake_luv = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: lunaverse_token.to_string(),
            funds: vec![],
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: lunaverse_gov.to_string(),
                amount: total_luv_stake_amount,
                msg: to_binary(&LunaverseStakingCw20HookMsg::Bond {})?,
            })?,
        });
        messages.push(stake_luv);
    }

    if !provide_luv.is_zero() {
        ...
        let stake = CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&ExecuteMsg::stake {
                asset_token: lunaverse_token.to_string(),
            })?,
            funds: vec![],
        });
        messages.push(stake);
    ...
    attributes.push(attr("asset_token", lunaverse_token));
    attributes.push(attr("reinvest_amount", reinvest_amount));
    attributes.push(attr("provide_token_amount", provide_luv));
...
pub fn stake(
    ...
    // only lunaverse farm contract can execute this message
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            funds: vec![],
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: lunaverse_staking.to_string(),
                amount,
                msg: to_binary(&LunaverseStakingCw20HookMsg::Bond {})?,
            })?,
        })])
