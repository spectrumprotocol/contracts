...
use crate::lunaverse_staking::ExecuteMsg as LunaverseStakingExecuteMsg;
...
const LUV_TOKEN: &str = "luv_token";
const LUV_STAKING: &str = "luv_staking";
const LUV_LP: &str = "luv_lp";
const LUV_POOL: &str = "luv_pool";
const LUV_GOV: &str = "luv_gov";
...
fn test() {
    ...
    deps.querier.with_terraswap_pairs(&[
        (
            &"uusdluv_token".to_string(),
            &PairInfo {
                asset_infos: [
                    AssetInfo::Token {
                        contract_addr: LUV_TOKEN.to_string(),
                    },
                    AssetInfo::NativeToken {
                        denom: "uusd".to_string(),
                    },
                ],
                contract_addr: LUV_POOL.to_string(),
                liquidity_token: LUV_LP.to_string(),
            },
        ),
        (
    ...
    test_compound_luv_from_allowance(&mut deps);
    test_bond(&mut deps);
    test_compound_luv(&mut deps);
    test_compound_luv_with_fees(&mut deps);
}
...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        lunaverse_token: LUV_TOKEN.to_string(),
        lunaverse_staking: LUV_STAKING.to_string(),
        lunaverse_gov: LUV_GOV.to_string(),
        ...
        pair_contract: LUV_POOL.to_string(),
    };
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    let msg = ExecuteMsg::register_asset {
        asset_token: LUV_TOKEN.to_string(),
        staking_token: LUV_LP.to_string(),
        weight: 1u32,
    };
    let res = execute(deps.as_mut(), env.clone(), info, msg.clone());
    assert!(res.is_err());
    ...
    assert_eq!(
        ...
        vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: LUV_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&LunaverseStakingExecuteMsg::Withdraw { }).unwrap(),
            }),
        ]
    );
}

fn test_compound_luv_from_allowance(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);

    deps.querier.with_token_balances(&[
        (&LUV_TOKEN.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(100_000_000u128))])
    ]);
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    // bond success user1 1000 LUV-LP
    let info = mock_info(LUV_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    let mut pool_info = pool_info_read(deps_ref.storage)
        .load(config.lunaverse_token.as_slice())
        .unwrap();
        deposit_farm_share(
            deps_ref,
            &env,
    ...
    pool_info_store(deps.as_mut().storage)
        .save(config.lunaverse_token.as_slice(), &pool_info)
        .unwrap();
    deps.querier.with_token_balances(&[
        (
            &LUV_STAKING.to_string(),
    ...
    assert_eq!(
        ...
        [
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: LUV_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&LunaverseStakingExecuteMsg::InstantUnbond {
                    amount: Uint128::from(3000u128),
                })
                .unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: LUV_LP.to_string(),
                funds: vec![],
    ...
    // bond user2 5000 LUV-LP auto-compound
    let info = mock_info(LUV_LP, &[]);
    let msg = ExecuteMsg::receive(Cw20ReceiveMsg {
...
fn test_compound_luv(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    /*
    pending rewards 12000 LUV
    USER1 7000 (auto 7000, stake 0)
    USER2 5000 (auto 5000, stake 0)
    total 12000
    auto 12000 / 12000 * 12000 = 12000
    stake 0 / 12000 * 12000 = 0
    swap amount 6000 LUV -> 5982 UST
    provide UST = 5863
    provide LUV = 5863
    remaining = 137
    */
...
fn test_compound_luv_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    // update fees
    let env = mock_env();
    let info = mock_info(SPEC_GOV, &[]);
    let msg = ExecuteMsg::update_config {
    ...
    swap amount 6532 LUV (12100 / 2 + 605) -> 6512 UST
    provide UST = 5616
    provide LUV = 5616
    remaining = 311
    fee swap amount 605 LUV -> 591 UST -> 590 SPEC
    community fee = 363 / 605 * 590 = 354
    platform fee = 121 / 605 * 590 = 118
    controller fee = 121 / 605 * 590 = 118
    total swap amount 6532 LUV
    */
    let msg = ExecuteMsg::compound {};
    let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
