...
use crate::querier::query_lunaverse_pool_balance;
use crate::lunaverse_staking::{Cw20HookMsg as LunaverseCw20HookMsg, ExecuteMsg as LunaverseExecuteMsg, StakerInfoResponse as LunaverseStakerInfoResponse};
...
pub fn bond(
    ...
    let lp_balance = query_lunaverse_pool_balance(
        deps.as_ref(),
...
pub fn deposit_farm_share(
    ...
    let staked = LunaverseStakerInfoResponse {
...
// stake LP token to Lunaverse Staking
fn stake_token(
    api: &dyn Api,
    ...
    let lunaverse_staking = api.addr_humanize(&lunaverse_staking)?;
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::Send {
                ...
                amount,
                msg: to_binary(&LunaverseCw20HookMsg::Bond {})?,
            })?,
        })])
        .add_attributes(vec![
            attr("action", "bond"),
    ...
    Ok(Response::new()
        .add_messages(vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps.api.addr_humanize(&config.lunaverse_staking)?.to_string(),
                funds: vec![],
                msg: to_binary(&LunaverseExecuteMsg::InstantUnbond { amount })?,
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps
                    .api
...
pub fn withdraw(
    ...
    if !farm_amount.is_zero() {
        messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps.api.addr_humanize(&config.lunaverse_gov)?.to_string(),
            msg: to_binary(&LunaverseExecuteMsg::InstantUnbond {
                    amount: farm_amount,
                }
            )?,
            funds: vec![],
        ...
        messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps.api.addr_humanize(&config.lunaverse_token)?.to_string(),
            msg: to_binary(&Cw20ExecuteMsg::Transfer {
                recipient: info.sender.to_string(),
                amount: farm_amount,
            })?,
...
fn withdraw_reward(
    ...
    let farm_staked = LunaverseStakerInfoResponse {
        staker: env.contract.address.to_string(),
        reward_index: Decimal::zero(),
        bond_amount: Uint128::zero(),
        pending_reward: Uint128::zero(),
...
fn read_reward_infos(
    ...
    let lp_balance =
        query_lunaverse_pool_balance(deps, &config.lunaverse_staking, &env.contract.address)?;
