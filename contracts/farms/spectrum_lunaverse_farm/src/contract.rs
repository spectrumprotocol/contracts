...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            lunaverse_token: deps.api.addr_canonicalize(&msg.lunaverse_token)?,
            lunaverse_staking: deps.api.addr_canonicalize(&msg.lunaverse_staking)?,
            lunaverse_gov: deps.api.addr_canonicalize(&msg.lunaverse_gov)?,
...
fn query_config(deps: Deps) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        lunaverse_token: deps.api.addr_humanize(&config.lunaverse_token)?.to_string(),
        lunaverse_staking: deps.api.addr_humanize(&config.lunaverse_staking)?.to_string(),
        spectrum_gov: deps.api.addr_humanize(&config.spectrum_gov)?.to_string(),
        lunaverse_gov: deps.api.addr_humanize(&config.lunaverse_gov)?.to_string(),
