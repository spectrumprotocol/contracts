...
use crate::lunaverse_staking::{StakerInfoResponse as LunaverseStakerInfoResponse};
        ...
        match &request {
            ...
            QueryRequest::Wasm(WasmQuery::Smart { contract_addr, msg }) => {
                match from_binary(msg).unwrap() {
                    ...
                    MockQueryMsg::StakerInfo { staker } => {
                        let contract_addr = &LUV_STAKING.to_string();
                        let balance = self.read_token_balance(contract_addr, staker.clone());
                        SystemResult::Ok(ContractResult::from(to_binary(
                            &LunaverseStakerInfoResponse {
                                staker,
                                bond_amount: balance,
