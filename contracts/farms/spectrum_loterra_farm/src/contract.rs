...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            loterra_token: deps.api.addr_canonicalize(&msg.loterra_token)?,
            loterra_staking: deps.api.addr_canonicalize(&msg.loterra_staking)?,
            loterra_gov: deps.api.addr_canonicalize(&msg.loterra_gov)?,
...
fn query_config(deps: Deps<TerraQuery>) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        loterra_token: deps.api.addr_humanize(&config.loterra_token)?.to_string(),
        loterra_staking: deps.api.addr_humanize(&config.loterra_staking)?.to_string(),
        spectrum_gov: deps.api.addr_humanize(&config.spectrum_gov)?.to_string(),
        loterra_gov: deps.api.addr_humanize(&config.loterra_gov)?.to_string(),
