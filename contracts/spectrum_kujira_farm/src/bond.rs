...
use spectrum_protocol::kujira_farm::{RewardInfoResponse, RewardInfoResponseItem};
...
pub fn bond(
    ...
    let lp_balance = query_kujira_pool_balance(
        deps.as_ref(),
...
pub fn deposit_farm_share(
    ...
    let staked = MockGovStakerInfoResponse {
...
// stake LP token to Kujira Staking
fn stake_token(
    api: &dyn Api,
    ...
    let kujira_staking = api.addr_humanize(&kujira_staking)?;
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::Send {
                ...
                amount,
                msg: to_binary(&KujiraCw20HookMsg::Bond {})?,
            })?,
        })])
        .add_attributes(vec![
            attr("action", "bond"),
    ...
    Ok(Response::new()
        .add_messages(vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps.api.addr_humanize(&config.kujira_staking)?.to_string(),
                funds: vec![],
                msg: to_binary(&KujiraStakingExecuteMsg::Unbond { amount })?,
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps
                    .api
...
fn withdraw_reward(
    ...
    let farm_staked = MockGovStakerInfoResponse {
        staker: deps.api.addr_humanize(&state.contract_addr)?.to_string(),
        reward_index: Decimal::zero(),
        bond_amount: Uint128::zero(),
        pending_reward: Uint128::zero(),
...
fn read_reward_infos(
    ...
    let lp_balance =
        query_kujira_pool_balance(deps, &config.kujira_staking, &state.contract_addr, env.block.time.seconds())?;
