...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            kujira_token: deps.api.addr_canonicalize(&msg.kujira_token)?,
            kujira_staking: deps.api.addr_canonicalize(&msg.kujira_staking)?,
...
fn query_config(deps: Deps) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        kujira_token: deps.api.addr_humanize(&config.kujira_token)?.to_string(),
        kujira_staking: deps.api.addr_humanize(&config.kujira_staking)?.to_string(),
