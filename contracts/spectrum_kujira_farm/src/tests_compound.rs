...
const KUJIRA_TOKEN: &str = "kujira_token";
const KUJIRA_STAKING: &str = "kujira_staking";
const KUJIRA_LP: &str = "kujira_lp";
const KUJIRA_POOL: &str = "kujira_pool";
...
fn test() {
    ...
    deps.querier.with_terraswap_pairs(&[
        (
            ...
            &PairInfo {
                ...
                contract_addr: KUJIRA_POOL.to_string(),
                liquidity_token: KUJIRA_LP.to_string(),
            },
        ),
        (
    ...
    test_compound_kujira_from_allowance(&mut deps);
    test_bond(&mut deps);
    test_compound_kujira(&mut deps);
    test_compound_kujira_with_fees(&mut deps);
}
...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        kujira_token: KUJIRA_TOKEN.to_string(),
        kujira_staking: KUJIRA_STAKING.to_string(),
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    let msg = ExecuteMsg::register_asset {
        ...
        staking_token: KUJIRA_LP.to_string(),
        weight: 1u32,
    };
    let res = execute(deps.as_mut(), env.clone(), info, msg.clone());
    assert!(res.is_err());
    ...
    assert_eq!(
        ...
        vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                funds: vec![],
                msg: to_binary(&KujiraStakingExecuteMsg::Withdraw {}).unwrap(),
            }),
        ]
    );
}

fn test_compound_kujira_from_allowance(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);

    let asset_token_raw = deps.api.addr_canonicalize(&KUJIRA_TOKEN.to_string()).unwrap();
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                msg: to_binary(&Cw20ExecuteMsg::Send {
                    contract: KUJIRA_POOL.to_string(),
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    // bond success user1 1000 KUJIRA-LP
    let info = mock_info(KUJIRA_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    let mut pool_info = pool_info_read(deps_ref.storage)
        .load(config.kujira_token.as_slice())
        .unwrap();
        deposit_farm_share(
            deps_ref,
            &mut state,
    ...
    pool_info_store(deps.as_mut().storage)
        .save(config.kujira_token.as_slice(), &pool_info)
        .unwrap();
    deps.querier.with_token_balances(&[
        (
            &KUJIRA_STAKING.to_string(),
    ...
    assert_eq!(
        ...
        [
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: KUJIRA_STAKING.to_string(),
                funds: vec![],
                msg: to_binary(&KujiraStakingExecuteMsg::Unbond {
                    amount: Uint128::from(3000u128),
                })
                .unwrap(),
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: KUJIRA_LP.to_string(),
                funds: vec![],
    ...
    // bond user2 5000 KUJIRA-LP auto-compound
    let info = mock_info(KUJIRA_LP, &[]);
    let msg = ExecuteMsg::receive(Cw20ReceiveMsg {
...
fn test_compound_kujira(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    /*
    pending rewards 12000 KUJIRA
    USER1 7000 (auto 7000, stake 0)
    USER2 5000 (auto 5000, stake 0)
    total 12000
    auto 12000 / 12000 * 12000 = 12000
    stake 0 / 12000 * 12000 = 0
    swap amount 6000 KUJIRA -> 5982 UST
    provide UST = 5863
    provide KUJIRA = 5863
    remaining = 137
    */
...
fn test_compound_kujira_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    // update fees
    let env = mock_env();
    let info = mock_info(SPEC_GOV, &[]);
    let msg = ExecuteMsg::update_config {
    ...
    swap amount 6532 KUJIRA (12100 / 2 + 605) -> 6512 UST
    provide UST = 5616
    provide KUJIRA = 5616
    remaining = 311
    fee swap amount 605 KUJIRA -> 591 UST -> 590 SPEC
    community fee = 363 / 605 * 590 = 354
    platform fee = 121 / 605 * 590 = 118
    controller fee = 121 / 605 * 590 = 118
    total swap amount 6532 KUJIRA
    */
    let msg = ExecuteMsg::compound {};
    let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
