...
use crate::querier::query_kujira_reward_info;
...
use kujira::staking::{
    Cw20HookMsg as KujiraStakingCw20HookMsg, ExecuteMsg as KujiraStakingExecuteMsg,
};
use spectrum_protocol::gov::{Cw20HookMsg as GovCw20HookMsg, ExecuteMsg as GovExecuteMsg};
use spectrum_protocol::kujira_farm::ExecuteMsg;
...
pub fn compound(deps: DepsMut, env: Env, info: MessageInfo) -> StdResult<Response> {
    ...
    let kujira_staking = deps.api.addr_humanize(&config.kujira_staking)?;
    let kujira_token = deps.api.addr_humanize(&config.kujira_token)?;
    ...
    let kujira_reward_info = query_kujira_reward_info(
        deps.as_ref(),
        &config.kujira_staking,
        &deps.api.addr_canonicalize(env.contract.address.as_str())?,
        Some(env.block.time.seconds()),
    )?;

    let mut total_kujira_swap_amount = Uint128::zero();
    let mut total_kujira_stake_amount = Uint128::zero();
    let mut total_kujira_commission = Uint128::zero();
    ...
    // calculate auto-compound, auto-Stake, and commission in KUJIRA
    let mut pool_info = pool_info_read(deps.storage).load(config.kujira_token.as_slice())?;
    let reward = kujira_reward_info.pending_reward;
    if !reward.is_zero() && !kujira_reward_info.bond_amount.is_zero() {
        let commission = reward * total_fee;
        let kujira_amount = reward.checked_sub(commission)?;
        // add commission to total swap amount
        total_kujira_commission += commission;
        total_kujira_swap_amount += commission;

        let auto_bond_amount = kujira_reward_info
            .bond_amount
            .checked_sub(pool_info.total_stake_bond_amount)?;
        compound_amount =
            kujira_amount.multiply_ratio(auto_bond_amount, kujira_reward_info.bond_amount);
        let stake_amount = kujira_amount.checked_sub(compound_amount)?;
        ...
        total_kujira_stake_amount += stake_amount;
    }
    ...
    // add commission to reinvest KUJIRA to total swap amount
    total_kujira_swap_amount += swap_amount;

    let kujira_pair_info = query_pair_info(
        &deps.querier,
    ...
    // find KUJIRA swap rate
    let kujira = Asset {
        info: AssetInfo::Token {
            contract_addr: kujira_token.to_string(),
        },
        amount: total_kujira_swap_amount,
    };
    let kujira_swap_rate = simulate(
        &deps.querier,
        deps.api.addr_validate(&kujira_pair_info.contract_addr)?,
        &kujira,
    )?;
    let return_asset = Asset {
        ...
        amount: kujira_swap_rate.return_amount,
    };

    let total_ust_return_amount = return_asset.deduct_tax(&deps.querier)?.amount;
    let total_ust_commission_amount = if total_kujira_swap_amount != Uint128::zero() {
        total_ust_return_amount.multiply_ratio(total_kujira_commission, total_kujira_swap_amount)
    } else {
        Uint128::zero()
    };
    ...
    let swap_kujira_rate = simulate(
        &deps.querier,
        deps.api.addr_validate(&kujira_pair_info.contract_addr)?,
        &net_reinvest_asset,
    )?;
    // calculate provided KUJIRA from provided UST
    let provide_kujira = swap_kujira_rate.return_amount + swap_kujira_rate.commission_amount;

    pool_info.reinvest_allowance = swap_amount.checked_sub(provide_kujira)?;
    pool_info_store(deps.storage).save(config.kujira_token.as_slice(), &pool_info)?;

    attributes.push(attr("total_ust_return_amount", total_ust_return_amount));

    let mut messages: Vec<CosmosMsg> = vec![];
    let withdraw_all_kujira: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: kujira_staking.to_string(),
        funds: vec![],
        msg: to_binary(&KujiraStakingExecuteMsg::Withdraw {})?,
    });
    messages.push(withdraw_all_kujira);

    if !total_kujira_swap_amount.is_zero() {
        let swap_kujira: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: kujira_token.to_string(),
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: kujira_pair_info.contract_addr.clone(),
                amount: total_kujira_swap_amount,
    ...
    if !provide_kujira.is_zero() {
        let increase_allowance = CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::IncreaseAllowance {
                spender: kujira_pair_info.contract_addr.to_string(),
                amount: provide_kujira,
                expires: None,
            })?,
            funds: vec![],
        });
        ...
        let provide_liquidity = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: kujira_pair_info.contract_addr,
        ...
        let stake = CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&ExecuteMsg::stake {
                asset_token: kujira_token.to_string(),
            })?,
            funds: vec![],
        });
        messages.push(stake);
    ...
    attributes.push(attr("asset_token", kujira_token));
    attributes.push(attr("reinvest_allowance", reinvest_allowance));
    attributes.push(attr("provide_token_amount", provide_kujira));
...
pub fn stake(
    ...
    // only kujira farm contract can execute this message
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            funds: vec![],
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: kujira_staking.to_string(),
                amount,
                msg: to_binary(&KujiraStakingCw20HookMsg::Bond {})?,
            })?,
        })])
        .add_attributes(vec![
            attr("action", "stake"),
