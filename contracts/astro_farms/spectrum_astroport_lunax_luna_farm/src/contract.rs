...
use crate::model::{ConfigInfo, Cw20HookMsg, ExecuteMsg, MigrateMsg, PoolItem, PoolsResponse, QueryMsg, StateInfo};
...
pub fn instantiate(
    ...
    store_config(
        ...
        &Config {
            ...
            sd_token: deps.api.addr_canonicalize(&msg.sd_token)?,
            ...
            sd_ust_pair_contract: deps.api.addr_canonicalize(&msg.sd_ust_pair_contract)?,
...
fn query_config(deps: Deps) -> StdResult<ConfigInfo> {
    ...
    let resp = ConfigInfo {
        ...
        sd_token: deps.api.addr_humanize(&config.sd_token)?.to_string(),
        lunax_token: deps.api.addr_humanize(&config.lunax_token)?.to_string(),
        ...
        sd_ust_pair_contract: deps.api.addr_humanize(&config.sd_ust_pair_contract)?.to_string(),
