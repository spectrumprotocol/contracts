...
const SD_TOKEN: &str = "sd_token";
const LUNAX_TOKEN: &str = "lunax_token";
...
const ULUNA_UUSD_PAIR_CONTRACT: &str = "uluna_uusd_pair_contract";
const SD_UST_PAIR_CONTRACT: &str = "sd_ust_pair_contract";
const ULUNA_UUSD_LP: &str = "uluna_uusd_lp";
const LUNAX_LUNA_LP: &str = "lunax_luna_lp";
...
fn test() {
    ...
    (
        ...
        &PairInfo {
            ...
            contract_addr: deps.api.addr_validate(ULUNA_UUSD_PAIR_CONTRACT).unwrap(),
            liquidity_token: deps.api.addr_validate(ULUNA_UUSD_LP).unwrap(),
    ...
    // compound ASTRO only because gov_proxy is set but no LUNAX_TOKEN in contract,
    // compound LUNAX_TOKEN and ASTRO,
    // compound LUNAX_TOKEN only
}
...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        sd_ust_pair_contract: SD_UST_PAIR_CONTRACT.to_string(),
        lunax_token: LUNAX_TOKEN.to_string(),
        uluna_uusd_pair_contract: ULUNA_UUSD_PAIR_CONTRACT.to_string()
    };
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    let msg = ExecuteMsg::register_asset {
        asset_token: LUNAX_TOKEN.to_string(),
        staking_token: LUNAX_LUNA_LP.to_string(),
        weight: 1u32,
    };
    let res = execute(deps.as_mut(), env.clone(), info, msg.clone());
    assert!(res.is_err());
    ...
    assert_eq!(
        ...
        vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&AstroportExecuteMsg::Withdraw {
                lp_token: deps.api.addr_validate(LUNAX_LUNA_LP).unwrap(),
                amount: Uint128::zero()
            })
            .unwrap(),
        }),]
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    let info = mock_info(LUNAX_LUNA_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    let mut pool_info = pool_info_read(deps_ref.storage)
        .load(config.lunax_token.as_slice())
        .unwrap();
    deposit_farm_share(
        deps_ref,
        &env,
    ...
    pool_info_store(deps.as_mut().storage)
        .save(config.sd_token.as_slice(), &pool_info)
        .unwrap();
    deps.querier.with_token_balances(&[
        (
    ...
    assert_eq!(
        ...
        [
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: LUNAX_LUNA_LP.to_string(),
                funds: vec![],
    ...
    // bond user2 5000 LUNAX_TOKEN -LP auto-stake
    let info = mock_info(LUNAX_LUNA_LP, &[]);
    let msg = ExecuteMsg::receive(Cw20ReceiveMsg {
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                msg: to_binary(&AstroportPairExecuteMsg::Swap {
                    ...
                    amount: Uint128::from(99009900u128),
                    }
                }).unwrap(),
                    ...
                    assets: [
                        Asset {
                            info: AssetInfo::Token {
                                contract_addr: deps.api.addr_validate(LUNAX_TOKEN).unwrap(),
                            },
                            amount: Uint128::zero(),
                        },
                        Asset {
