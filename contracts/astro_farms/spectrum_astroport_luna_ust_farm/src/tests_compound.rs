...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        xastro_proxy: ASTRO_GOV_PROXY.to_string(),
        ...
        farm_denom: FARM_DENOM.to_string(),
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    ...
    let msg = ExecuteMsg::register_asset {
        asset_token: FARM_DENOM.to_string(),
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    ...
    let mut pool_info = pool_info_read(deps_ref.storage)
        .load(config.farm_denom.as_bytes())
        .unwrap();
    deposit_farm_share(
        deps_ref,
        &env,
    ...
    pool_info_store(deps.as_mut().storage)
        .save(config.farm_denom.as_bytes(), &pool_info)
        .unwrap();
    deps.querier.with_token_balances(&[
        (
            &ASTROPORT_GENERATOR.to_string(),
...
fn test_compound_astro(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    let env = mock_env();
    let info = mock_info(TEST_CONTROLLER, &[]);
    ...
    assert_eq!(
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                funds: vec![
                    Coin { denom: "uusd".to_string(), amount: Uint128::from(2055u128) }
                ],
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: ASTRO_TOKEN.to_string(),
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                    ...
                    assets: [
                        Asset {
                            info: AssetInfo::NativeToken {
                                denom: FARM_DENOM.to_string(),
                            },
                            amount: Uint128::from(2049u128),
                        },
                        Asset {
                            info: AssetInfo::NativeToken {
                                denom: "uusd".to_string(),
                ...
                funds: vec![
                    Coin { denom: FARM_DENOM.to_string(), amount: Uint128::from(2049u128) },
                    Coin { denom: "uusd".to_string(), amount: Uint128::from(2049u128) },
                ],
            })
        ]
    );
...
fn test_compound_astro_with_fees(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier, TerraQuery>) {
    // update fees
    let env = mock_env();
    let info = mock_info(SPEC_GOV, &[]);
    let msg = ExecuteMsg::update_config {
    ...
    assert_eq!(
        ...
        vec![
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                funds: vec![
                    Coin { denom: "uusd".to_string(), amount: Uint128::from(1999u128) }
                ],
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                msg: to_binary(&AstroportPairExecuteMsg::ProvideLiquidity {
                    assets: [
                        Asset {
                            ...
                            amount: Uint128::from(1994u128),
                        },
                        Asset {
                            info: AssetInfo::NativeToken {
                                denom: "uusd".to_string(),
                            },
                            amount: Uint128::from(1993u128),
                        },
                    ],
                    slippage_tolerance: None,
                    auto_stake: Some(true),
                ...
                funds: vec![
                    Coin { denom: FARM_DENOM.to_string(), amount: Uint128::from(1994u128) },
                    Coin { denom: "uusd".to_string(), amount: Uint128::from(1993u128) }
                ],
            }),
        ]
    );
