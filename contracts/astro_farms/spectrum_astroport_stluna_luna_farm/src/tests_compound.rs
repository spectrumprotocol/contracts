...
const STLUNA_WELDO_PAIR_CONTRACT: &str = "stluna_weldo_pair_contract";
const ULUNA_UUSD_LP: &str = "uluna_uusd_lp";
const STLUNA_LUNA_LP: &str = "stluna_luna_lp";
...
fn test() {
    ...
    deps.querier.with_astroport_pairs(&[(
        ...
        &PairInfo {
            ...
            contract_addr: deps.api.addr_validate(STLUNA_WELDO_PAIR_CONTRACT).unwrap(),
...
fn test_config(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) -> ConfigInfo {
    ...
    let mut config = ConfigInfo {
        ...
        stluna_weldo_pair_contract: STLUNA_WELDO_PAIR_CONTRACT.to_string(),
...
fn test_register_asset(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    let msg = ExecuteMsg::register_asset {
        ...
        staking_token: STLUNA_LUNA_LP.to_string(),
        weight: 1u32,
    };
    let res = execute(deps.as_mut(), env.clone(), info, msg.clone());
    assert!(res.is_err());
    ...
    assert_eq!(
        ...
        vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&AstroportExecuteMsg::Withdraw {
                lp_token: deps.api.addr_validate(STLUNA_LUNA_LP).unwrap(),
                amount: Uint128::zero()
            })
            .unwrap(),
        }),]
...
fn test_bond(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    let info = mock_info(STLUNA_LUNA_LP, &[]);
    let res = execute(deps.as_mut(), env.clone(), info, msg);
    assert!(res.is_ok());
    ...
    assert_eq!(
        ...
        [
            ...
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: STLUNA_LUNA_LP.to_string(),
                funds: vec![],
