use cosmwasm_std::{
    ...
    Order, Querier, QuerierWrapper, QueryRequest, Response, StdError, StdResult, Storage, Uint128,
    WasmMsg, WasmQuery,
};
...
pub fn deposit_farm_share(
    ...
    let staked: AnchorStakerResponse = querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
...
pub fn withdraw(
    ...
    use cosmwasm_std::{
        attr, to_binary, Api, CanonicalAddr, CosmosMsg, Decimal, Deps, DepsMut, Env, MessageInfo,
        Order, Querier, QuerierWrapper, QueryRequest, Response, StdError, StdResult, Storage,
    ...
    use spectrum_protocol::anchor_farm::{RewardInfoResponse, RewardInfoResponseItem};
    use spectrum_protocol::gov::{
        BalanceResponse as SpecBalanceResponse, ExecuteMsg as SpecExecuteMsg,
        QueryMsg as SpecQueryMsg,
    };
    use spectrum_protocol::math::UDec128;
    ...
    fn spec_reward_to_pool(
        ...
        let share = (UDec128::from(state.spec_share_index)
            - pool_info.state_spec_share_index.into())
            * Uint128::from(pool_info.weight as u128);
        let stake_share = share * pool_info.total_stake_bond_amount / lp_balance;
    ...
    pub fn unbond(
        ...
        reward_info.auto_bond_share = reward_info.auto_bond_share.checked_sub(auto_bond_share)?;
        reward_info.stake_bond_share =
            reward_info.stake_bond_share.checked_sub(stake_bond_share)?;
    ...
    pub fn withdraw(
        ...
        for reward_pair in reward_pairs {
            ...
            let locked_share =
                config.calc_locked_reward(reward_info.accum_spec_share, env.block.height);
        ...
        let reward_infos: Vec<RewardInfoResponseItem> = reward_pair
            ...
            .map(|(asset_token_raw, reward_info)| {
                ...
                let auto_bond_amount =
                    pool_info.calc_user_auto_balance(lp_balance, reward_info.auto_bond_share);
                let stake_bond_amount =
                    pool_info.calc_user_stake_balance(reward_info.stake_bond_share);
