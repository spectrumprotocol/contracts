...
use cosmwasm_std::{Uint128, to_binary, from_binary, OwnedDeps};
...
fn test_stake(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    deps.querier.with_token_balances(&[
        (&SPEC_GOV.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(100u128))])
    ]);
    ...
    deps.querier.with_token_balances(&[
        (&SPEC_GOV.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(80u128))])
    ]);
...
fn test_reward(deps: &mut OwnedDeps<MockStorage, MockApi, WasmMockQuerier>) {
    ...
    deps.querier.with_token_balances(&[
        (&SPEC_GOV.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(121u128))])
    ]);
    let msg = QueryMsg::balance {
        address: USER1.to_string(),
