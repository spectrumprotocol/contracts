...
pub fn compound(deps: DepsMut, env: Env, info: MessageInfo) -> StdResult<Response> {
    ...
    let vkr_pair_info = query_pair_info(
        &deps.querier,
    ...
    let vkr_swap_rate = simulate(
        &deps.querier,
        deps.api.addr_validate(&vkr_pair_info.contract_addr)?,
        &vkr,
    )?;
    let return_asset = Asset {
        ...
        amount: vkr_swap_rate.return_amount,
    };
    ...
    let swap_vkr_rate = simulate(
        &deps.querier,
        deps.api.addr_validate(&vkr_pair_info.contract_addr)?,
        &net_reinvest_asset,
    )?;
    // calculate provided VKR from provided UST
    let provide_vkr = swap_vkr_rate.return_amount + swap_vkr_rate.commission_amount;

    pool_info.reinvest_allowance = swap_amount.checked_sub(provide_vkr)?;
    ...
    if !total_vkr_swap_amount.is_zero() {
        let swap_vkr: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::Send {
                contract: vkr_pair_info.contract_addr.clone(),
                amount: total_vkr_swap_amount,
    ...
    if !provide_vkr.is_zero() {
        let increase_allowance = CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::IncreaseAllowance {
                spender: vkr_pair_info.contract_addr.to_string(),
                amount: provide_vkr,
                expires: None,
            })?,
            funds: vec![],
        ...
        let provide_liquidity = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: vkr_pair_info.contract_addr,
