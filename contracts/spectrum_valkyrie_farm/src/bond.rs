...
use crate::querier::query_valkyrie_pool_balance;
use valkyrie::governance::execute_msgs::{
    ExecuteMsg as ValkyrieGovExecuteMsg
};
use valkyrie::governance::query_msgs::{
    QueryMsg as ValkyrieGovQueryMsg,
    StakerStateResponse as ValkyrieStakerResponse,
};
use valkyrie::lp_staking::execute_msgs::{
    Cw20HookMsg as ValkyrieCw20HookMsg, ExecuteMsg as ValkyrieStakingExecuteMsg,
};
use spectrum_protocol::valkyrie_farm::{RewardInfoResponse, RewardInfoResponseItem};
...
pub fn deposit_farm_share(
    ...
    let staked: ValkyrieStakerResponse =
...
// stake LP token to Valkyrie Staking
fn stake_token(
    api: &dyn Api,
    ...
    let valkyrie_staking = api.addr_humanize(&valkyrie_staking)?;
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::Send {
                ...
                amount,
                msg: to_binary(&ValkyrieCw20HookMsg::Bond {})?,
            })?,
        })])
        .add_attributes(vec![
            attr("action", "bond"),
    ...
    Ok(Response::new()
        .add_messages(vec![
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps.api.addr_humanize(&config.valkyrie_staking)?.to_string(),
                funds: vec![],
                msg: to_binary(&ValkyrieStakingExecuteMsg::Unbond { amount })?,
            }),
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps
                    .api
...
fn withdraw_reward(
    ...
    let farm_staked: ValkyrieStakerResponse =
        deps.querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
            contract_addr: deps.api.addr_humanize(&config.valkyrie_gov)?.to_string(),
            msg: to_binary(&ValkyrieGovQueryMsg::StakerState {
                address: deps.api.addr_humanize(&state.contract_addr)?.to_string(),
