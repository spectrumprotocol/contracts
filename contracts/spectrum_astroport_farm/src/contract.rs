...
use crate::{bond::bond, harvest::harvest_all, querier::query_pool_balance, reinvest::{re_invest, stake}, state::{read_config, state_store, store_config, Config, PoolInfo, State}};
...
pub fn register_asset(
    ...
    gov_proxy: Option<String>,
    reward_token: String,
    weight: u32,
) -> StdResult<Response> {
    let config: Config = read_config(deps.storage)?;
    let asset_token_raw = deps.api.addr_canonicalize(&asset_token)?;
    let staking_token_raw = deps.api.addr_canonicalize(&staking_token)?;
    ...
    let gov_proxy_to_be_inserted: Option<CanonicalAddr> = if gov_proxy.is_some(){
        Some(deps.api.addr_canonicalize(&gov_proxy.unwrap()).unwrap())
    } else {
        None
    };
    ...
    let mut pool_info = pool_info_read(deps.storage)
        ...
        .unwrap_or_else(|| PoolInfo {
            ...
            gov_proxy: gov_proxy_to_be_inserted,
            ...
            reinvest_allowance_astro: Uint128::zero(),
        });
