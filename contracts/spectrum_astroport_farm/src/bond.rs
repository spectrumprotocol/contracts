...
use crate::querier::{query_gov_balance, query_pool_balance};
use crate::state::{
...
use spectrum_protocol::astroport_farm::{RewardInfoResponse, RewardInfoResponseItem};
...
pub fn bond(
    ...
    let lp_balance = query_pool_balance(
        deps.as_ref(),
    ...
    //TODO update reinvest_allowance ANC, ASTRO
}

pub fn deposit_farm_share(
    deps: DepsMut,
    config: &Config,
    pool_pairs: Vec<(String, Uint128)>, // array of (pool address, new MIR amount) //TODO needed for Psi, nLuna, nEth?
) -> StdResult<()> {
    let mut state = read_state(deps.storage)?;

    let staked: StakerInfoGovResponse = query_gov_balance(deps, gov_proxy, staker, &config.astroport_generator);
...
fn withdraw_reward(
    ...
    let farm_staked: StakerInfoGovResponse = query_gov_balance(deps, gov_proxy, staker, &config.astroport_generator);;
